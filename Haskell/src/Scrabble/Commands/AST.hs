...
instance FromSExpr ScrabbleExp where
  fromSExpr = f where
    ...
    f (List [AtomSym "place", AtomSym s, o, p, AtomSym b]) =
      Place <$> (Placement s <$> fromSExpr o <*> fromSExpr p <*> pure (Just b))
    f (List [AtomSym "place", AtomSym s, o, p]) =
      Place <$> (Placement s <$> fromSExpr o <*> fromSExpr p <*> pure Nothing)
...
instance FromSExpr PrimSearchExp where
  fromSExpr = f where
    f (List [AtomSym "startsWith", AtomSym p]) = return $ StartsWith p
    f (List [AtomSym "endsWith",   AtomSym p]) = return $ EndsWith p
    ...
    f (List [AtomSym "lookslike",  AtomSym p]) = return $ LooksLike p
...
primToSearch1 (StartsWith pat) = return $ startsWith pat
primToSearch1 (EndsWith   pat) = return $ endsWith pat
...
primToSearch1 (LooksLike pat)  = return $ looksLike pat
